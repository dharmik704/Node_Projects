...
module.exports.checklogin = async (req,res) => {
    try{
        ...
        if(userdata.email == payload.email && userdata.password == payload.password){
...
module.exports.editprofile = async (req,res) => {
    try{
        ...
        if(userdata){
            ...
            if(updata){
                req.flash('success', 'Profile is Updated Successfully!! Please logout and login again...');
    ...
    try{
        ...
        if(info){
            const info = await transporter.sendMail({
                from: 'dharmikchhodvdiya@gmail.com', // sender address
                to: req.body.email, // list of receivers
                subject: `Appointment Confirmed: ${req.body.name} with ${ docdata.name }`, // Subject line
                text: "Hello world?", // plain text body
            ...
            const info1 = await transporter.sendMail({
                ...
                subject: `New Appointment: ${req.body.name} on ${req.body.date} Dear ${ docdata.name }`, // Subject line
                text: "Hello world?", // plain text body
...
module.exports.viewapointment = async (req,res) => {
    try{
        let appdata = await appointment.find({userid: req.user._id}).sort({_id: -1}).populate({
                path: "doctorid",
                model: doctor,
                populate:
                  {
