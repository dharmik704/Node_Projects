...
module.exports.checklogin = async (req,res) => {
    try{
        ...
        if(admindata.email == payload.email && admindata.password == payload.password){
...
module.exports.viewadmin = async (req,res) => {
    try{
        let admindata = await admin.find({}).sort({_id:-1});
        if(admindata){
        ...
        if(req.body.password == req.user.password){
            if(req.body.password != req.body.npass){
                if(req.body.npass == req.body.cpass){
                    let upass = await admin.findByIdAndUpdate(req.user._id, {password: req.body.npass});
                    if(upass){
