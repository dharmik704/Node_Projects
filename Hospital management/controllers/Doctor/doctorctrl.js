...
const patient = require("../../models/patientmdl");
...
module.exports.checklogin = async (req,res) => {
    try{
        const docdata = await doctor.findOne({email: req.body.email}).populate('specializationid').exec();
        const payload = {
            email: req.body.email,
            password: req.body.password,
            userdata: docdata
        }
        if(docdata.email == payload.email && docdata.password == payload.password){
...
module.exports.editprofile = async (req,res) => {
    try{
        ...
        if(ddata){
            if(req.file){
                imagepath = path.join(__dirname,'../..',ddata.doctorimg);
                await fs.unlinkSync(imagepath);
                req.body.doctorimg = doctor.ipath+'/'+req.file.filename;
            }
            else{
                req.body.doctorimg = ddata.doctorimg;
            }
            req.body.name = req.body.firstname+' '+req.body.lastname;
            let uprofile = await doctor.findByIdAndUpdate(req.params.id,req.body);
            if(uprofile){
                req.flash('success', 'Your Profile is Updated Successfully!! Please logout and login again...');
        ...
        if(req.body.password == req.user.password){
            if(req.body.password != req.body.npass){
                if(req.body.npass == req.body.cpass){
                    let upass = await doctor.findByIdAndUpdate(req.user._id, {password: req.body.npass});
                    if(upass){
...
module.exports.viewappointment = async (req,res) => {
    try{
        let appdata = await appointment.find({doctorid: req.user._id}).sort({_id: -1});
        if(appdata){
...
module.exports.insertpatient = async (req,res) => {
    try{
        ...
        if(appdata){
            req.body.name = req.body.firstname+' '+req.body.lastname;
            req.body.appointmentid = appdata.id;
            req.body.status = true;
            req.body.role = 'Patient';
            req.body.created_date = moment().format('LLL');
            let pdata = await patient.create(req.body);
            if(pdata){
...
module.exports.viewpatients = async (req,res) => {
    try{
        let pdata = await patient.find({}).sort({_id: -1}).populate('appointmentid').exec();
        if(pdata){
            return res.render('Doctor/doctor_viewpatients',{
                pdata
            });
        }
        else{
...
module.exports.showpatientdetails = async (req,res) => {
    try{
        let Patient = await patient.findById(req.params.id).populate('appointmentid').exec();
        if(Patient){
            return res.render('Doctor/doctor_viewpatientdetails',{
                Patient
            });
        }
        else{
